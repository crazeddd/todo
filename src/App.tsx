...
const App: Component = () => {
  const { createItem, getItems, items } = handleTodo();
    ...
    <>
      {isOpen() && <div class="modal secondary column gp-3">
        ...
        <input
          ...
          placeholder='Name'
          value={name()}
          onChange={(self) => setName(self.currentTarget.value)}
        ></input>
        <input
          type="date"
          placeholder='Date'
          value={date()}
          onChange={(self) => setDate(self.currentTarget.value)}
        ></input>
      ...
      <div class="wrapper secondary gp-2">
        <div class="row nav center">
          ...
          <div class="button-wrapper row">
            <button class="" onclick={() => setIsOpen(true)}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
        ...
        <div class="todo">
          <For each={items()}>
            {(item: { name: string, date: Date }, index) =>
              <div id={String({ index })} class="todo-item center">
                <p><b>{item.name}</b></p>
